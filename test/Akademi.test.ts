...
describe("POST /api/akademis", () => {
    ...
    it("should be able get akademi", async () => {
        ...
        expect(response.body.data.aktive).toBe(akademi.aktive)
    })
...
describe("GetBy Column /api/akademis/kolomName/:kolomName", () => {
    beforeEach(async () => {
        await UserTest.create()
        await AkademiTest.create()
    })
    ...
    it("should be able to get by kolom : ID", async () => {
        const response = await supertest(web)
            .get("/api/Akademis/nama/test")
            .set("X-API-TOKEN", "test")
        const responseID = await supertest(web)
            .get("/api/Akademis/id/" + response.body.data[0].id)
            .set("X-API-TOKEN", "test")
    ...
    it("should not be able to get by kolom : ID", async () => {
        const response = await supertest(web)
            ...
            .set("X-API-TOKEN", "test")
        const responseID = await supertest(web)
            .get("/api/Akademis/id/" + response.body.data[0].idxx)
            .set("X-API-TOKEN", "test")
